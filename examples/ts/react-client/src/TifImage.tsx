...
import { Metadata, Bitdepth } from "tiff-dec";
...
const fetchAsync = async () => {
	...
	const image = await fetch("./m51.tif")
		.then((tif) => {
			if (tif.status < 299) return tif.arrayBuffer();
			else throw new Error("could not recieve tif");
		})
...
const scale = (
	num: number,
	oldRange: ContrastParams,
	newRange: ContrastParams,
): number => {
	let scale = Math.floor(
		((num - oldRange.min) * (newRange.max - newRange.min)) /
			(oldRange.max - oldRange.min) +
			newRange.min,
	);
	if (scale > newRange.max) return newRange.max;
	if (scale < newRange.min) return newRange.min;
	return scale;
};
...
function TifImage() {
	...
	function preDepth(depth: Bitdepth, wasm: ModuleType): ContrastParams {
		switch (depth) {
	...
	useEffect(() => {
		const fetchFiles = async () => {
			try {
				...
				const initialDecodedImageData =
					wasmModule.to_decoded_u16(initialDecodedImage);

				let imageHistoMinMax: ContrastParams = preDepth(
					loadedMetadata.bit_depth,
					wasmModule,
				);
	...
	useEffect(() => {
		if (loaded) {
			...
			image = image?.map((pixel) => {
				return stretch(
					pixel,
					{ min: contrastSliderValue[0], max: contrastSliderValue[1] },
					range,
				);
			});
			setDisplayImage(image);
		}
	}, [contrastSliderValue, decodedImage, imageContrast, loaded]);
		...
		<div>
			{loaded && displayImage && decodedImage ? (
				...
				<div className="TifImage">
					<p>Not Loaded Yet</p>
					<p>{error?.error.stack.toString()}</p>
				</div>
			)}
		</div>
	);
